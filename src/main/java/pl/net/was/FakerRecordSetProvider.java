    ...
    private Object generateTimeWithTimeZone(Range range, TimeWithTimeZoneType timeType)
    {
        if (timeType.getPrecision() <= TimeWithTimeZoneType.MAX_SHORT_PRECISION) {
            ...
            long nanos = faker.number().numberBetween(
                    roundDiv(range.getLowValue().map(v -> unpackTimeNanos((long) v)).orElse(0L), factor) + (!range.isLowUnbounded() && !range.isLowInclusive() ? 1 : 0),
                    roundDiv(range.getHighValue().map(v -> unpackTimeNanos((long) v)).orElse(NANOSECONDS_PER_DAY), factor) + (!range.isHighUnbounded() && range.isHighInclusive() ? 1 : 0)) * factor;
